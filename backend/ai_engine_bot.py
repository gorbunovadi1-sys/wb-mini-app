...
def build_dispatcher(mini_app_url: str = None) -> Dispatcher:
    ...
    @dp.message(Command("testnormquery"))
    async def test_normquery(message: Message):
        """TEMPORARY — verifies POST /adv/v0/normquery/stats live against
        one of the admin's own WB cabinets. WB's own method description
        claims this endpoint only works for "cpm" campaigns, but its
        response schema explicitly documents cpc-specific null behavior for
        several fields — this settles which is actually true before any
        Кластеры/фразы feature gets built on top of it. Remove once verified
        (see project memory for the outcome)."""
        if not _is_admin(message.from_user.id):
            return
        user_id = cabinets.get_or_create_user(message.from_user.id)
        my_wb = [c for c in cabinets.list_cabinets(user_id) if c["marketplace"] == "wb"]
        if not my_wb:
            await message.answer("У тебя нет подключённого WB-кабинета для проверки.")
            return
        credentials = cabinets.get_credentials(my_wb[0]["id"])
        client = WBClient(credentials["api_key"])
        await message.answer(f"Проверяю normquery/stats на кабинете «{my_wb[0]['display_name'] or my_wb[0]['id']}»…")
        ...
        def _run() -> str:
            ...
            advert_ids = client.get_active_campaign_ids(changed_since=since)
            if not advert_ids:
                return "Нет активных кампаний за последние 30 дней — не на чем проверить."
            details = client.get_campaign_details(advert_ids[:20])
            lines = [f"Активных кампаний: {len(advert_ids)}, деталей получено: {len(details)}"]
            ...
            if not pairs_by_payment:
                sample = json.dumps(details[0], ensure_ascii=False)[:1200] if details else "нет деталей вообще"
                return "\n".join(lines) + "\n\nНе нашла nm_id в деталях кампаний — сырой пример:\n" + sample

            date_from = (today - datetime.timedelta(days=7)).isoformat()
            date_to = today.isoformat()
            for payment, items in pairs_by_payment.items():
                lines.append(f"\npaymentType={payment}: {len(items)} пар(ы) advert_id+nm_id")
                try:
                    stats = client.get_normquery_stats(items[:5], date_from, date_to)
                    lines.append(f"Ответ: {len(stats)} записей")
                    lines.append(json.dumps(stats[:2], ensure_ascii=False, indent=1)[:1200])
                except Exception as e:
                    body = getattr(getattr(e, "response", None), "text", "")[:300]
                    lines.append(f"ОШИБКА {type(e).__name__}: {e} {body}")
            return "\n".join(lines)
        ...
        for i in range(0, len(result), 3500):
            await message.answer(result[i:i + 3500])
