...
def _compute_fixed_totals_sync(cabinet_id: int, date_from: str, date_to: str):
    """Uses cached postings (already carry financial_data — no live posting
    fetch needed, avoids Ozon's current rate-limiting) for revenue/buyouts
    through the just-shipped customer_price fix, and a fresh LIVE accrual
    fetch through the buyout-filtering fix, so the before/after can be
    compared against Дарья's real reconciled August total (975,175.31₽)
    without needing a full cache rebuild."""
    ...
    # Delivery is shipped-scoped (wider than buyouts — see
    # ozon_margin.accrual_scope_sets), so sum it from ALL of per_offer_accrual
    # directly rather than only offers with buyout revenue.
