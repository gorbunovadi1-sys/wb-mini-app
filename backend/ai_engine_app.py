...
@app.get("/api/_debug/ozon-august-categories/{cabinet_id}")
def debug_ozon_august_categories(cabinet_id: int, telegram_id: int, date: str):
    """TEMPORARY — August's own accrual export reconciles to the real
    balance exactly (975,175.31₽) when EVERY line is summed, including many
    small Услуги партнёров/Другие услуги categories we don't currently
    capture. Checking whether those show up as NON_ITEM entries we're
    already summing but under-counting, or as a category we've never seen.
    One live call. Remove after use."""
    if telegram_id != int(os.environ.get("ADMIN_TELEGRAM_ID", "0")):
        raise HTTPException(status_code=403, detail="admin only")
    ...
    for a in accruals:
        ...
        if cat == "NON_ITEM":
            ...
            try:
                non_item_total_amount += float(amt or 0)
            except (TypeError, ValueError):
                pass
            if len(non_item_samples) < 40:
                non_item_samples.append(a)
    return {
        "date": date,
        "total_entries": len(accruals),
        "category_counts": dict(cat_counts),
        "non_item_total_amount": round(non_item_total_amount, 2),
        "non_item_samples": non_item_samples,
    }
